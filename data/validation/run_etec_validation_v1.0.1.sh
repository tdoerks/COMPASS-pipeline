#!/bin/bash
#SBATCH --job-name=etec_val_v1.0.1_fasta_fix
#SBATCH --output=/homes/tylerdoe/slurm-etec-validation-v1.0.1-fasta-fix-%j.out
#SBATCH --error=/homes/tylerdoe/slurm-etec-validation-v1.0.1-fasta-fix-%j.err
#SBATCH --time=4:00:00
...
echo "=========================================="
echo "COMPASS v1.0.1 ETEC Validation (FASTA Input Fix)"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if [ -f "data/validation/etec_results_v1.0.1/compass_summary.html" ]; then
        echo "  ✅ Summary HTML generated successfully"

        # Check for disclaimer
        if grep -q "disclaimer" data/validation/etec_results_v1.0.1/compass_summary.html; then
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Review summary report: data/validation/etec_results_v1.0.1/compass_summary.html"
    ...
    echo "Check logs: /homes/tylerdoe/slurm-etec-validation-v1.0.1-fasta-fix-${SLURM_JOB_ID}.out"
    echo ""
    exit 1
fi
