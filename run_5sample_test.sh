...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "=========================================="
    echo "Enhanced COMPASS Summary Report"
    echo "=========================================="
    echo ""
    echo "NOTE: The enhanced report is now automatically generated"
    echo "      as the final step of the COMPASS pipeline!"
    echo ""

    # Check if report was generated by the pipeline
    if [ -f "$OUTDIR/summary/compass_summary.html" ]; then
        echo "✅ Enhanced report generated successfully by pipeline!"
        REPORT_EXIT=0
    else
        echo "⚠️  Report not found at expected location, checking alternate paths..."
        # Check alternate locations
        if [ -f "$OUTDIR/compass_summary.html" ]; then
            echo "✅ Found report at: $OUTDIR/compass_summary.html"
            REPORT_EXIT=0
        else
            echo "❌ Report not found - this may indicate a pipeline issue"
            REPORT_EXIT=1
        fi
    fi
    ...
    if [ $REPORT_EXIT -eq 0 ]; then
        echo "✅ Pipeline and report generation succeeded!"
        echo ""
        ...
        if [ -f "$OUTDIR/summary/compass_summary.html" ]; then
            ls -lh "$OUTDIR/summary/compass_summary.html" "$OUTDIR/summary/compass_summary.tsv" 2>/dev/null
            REPORT_PATH="$OUTDIR/summary/compass_summary.html"
        else
            ls -lh "$OUTDIR/compass_summary.html" "$OUTDIR/compass_summary.tsv" 2>/dev/null
            REPORT_PATH="$OUTDIR/compass_summary.html"
        fi
        echo ""
        echo "🔍 To Download and Review:"
        echo "  scp tylerdoe@beocat.ksu.edu:$REPORT_PATH ."
        echo ""
    ...
    echo "2. ✅ Report generation now INTEGRATED into pipeline!"
    echo "   → Automatically runs as final pipeline step"
    echo "   → No manual report generation needed"
    echo "3. If issues found in the report:"
    echo "   → Note specific problems"
    echo "   → Debug and fix issues"
    echo "   → Re-run pipeline to test fixes"
    echo ""
