#!/bin/bash
#SBATCH --job-name=ecoli_2024_v1.2
#SBATCH --output=/fastscratch/tylerdoe/ecoli_2024_v1.2_%j.out
#SBATCH --error=/fastscratch/tylerdoe/ecoli_2024_v1.2_%j.err
#SBATCH --time=168:00:00
#SBATCH --cpus-per-task=2
#SBATCH --mem=8G
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=tdoerks@vet.k-state.edu

echo "=========================================="
echo "COMPASS Pipeline v1.2-dev - E. coli 2024"
echo "With: BUSCO + Fixed versions.yml race condition"
echo "=========================================="
echo "Job ID: $SLURM_JOB_ID"
echo "Start time: $(date)"
echo ""

# Change to fastscratch directory
cd /fastscratch/tylerdoe/COMPASS-pipeline

# Ensure we're on v1.2-dev branch
echo "Switching to v1.2-dev branch..."
git fetch origin
git checkout v1.2-dev
git pull origin v1.2-dev

# Load Nextflow
module load Nextflow

# Set unique Nextflow home to avoid cache conflicts
export NXF_HOME=/fastscratch/tylerdoe/.nextflow_ecoli_2024

# Run pipeline for all E. coli 2024 NARMS data with v1.2 features
# - BUSCO enabled for assembly quality assessment
# - Fixed versions.yml race condition
nextflow run main.nf \
    -profile beocat \
    --input_mode metadata \
    --filter_organism "Escherichia" \
    --filter_year_start 2024 \
    --filter_year_end 2024 \
    --skip_busco false \
    --outdir /fastscratch/tylerdoe/results_ecoli_all_2024 \
    -w work_ecoli_all_2024 \
    -name ecoli_2024_v12_${SLURM_JOB_ID} \
    -resume

EXIT_CODE=$?

echo ""
echo "=========================================="
echo "End time: $(date)"
echo "Exit code: $EXIT_CODE"
echo "=========================================="

if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ Pipeline completed successfully!"
    echo ""
    echo "v1.2 Features:"
    echo "  - BUSCO assembly quality metrics"
    echo "  - QUAST assembly statistics"
    echo "  - Fixed versions.yml race condition"
    echo "  - Enhanced report with 6 visualizations"
    echo ""
    echo "Results: /fastscratch/tylerdoe/results_ecoli_all_2024"
    echo ""
    echo "Generate enhanced report:"
    echo "  python bin/generate_report_v3.py \\"
    echo "    /fastscratch/tylerdoe/results_ecoli_all_2024 \\"
    echo "    -o /fastscratch/tylerdoe/results_ecoli_all_2024/summary/compass_report_enhanced.html \\"
    echo "    --prophage-metadata /homes/tylerdoe/databases/prophage_metadata.xlsx"
else
    echo "❌ Pipeline failed with exit code $EXIT_CODE"
    echo "Check logs: /fastscratch/tylerdoe/ecoli_2024_v1.2_${SLURM_JOB_ID}.err"
fi

exit $EXIT_CODE
